...
cv::Mat& cStreamingSender::Update(ICT_META_DATA* MetaData)
{
	...
	//OBJECT_DATA* rcvObject = new OBJECT_DATA;
	//ICT_META_DATA MetaData;
	MetaData->CameraNumber = packet->CameraNumber;
	MetaData->FrameNumber = packet->FrameNumber;
	MetaData->NumberOfObject = packet->NumberOfObject;
	MetaData->TechnologyType = packet->TechnologyType;
	MetaData->pObjectData = new OBJECT_DATA[packet->NumberOfObject];
	if (packet->chunkSize == 1)
	{
		packet->pObjectData = m_rcvBuffer + sizeof(sStreamingData);
		MetaData->pObjectData = (OBJECT_DATA*)packet->pObjectData;

		packet->data = m_rcvBuffer + sizeof(sStreamingData) + sizeof(OBJECT_DATA) * packet->NumberOfObject;
	...
	/*CString msg;
	msg.Format(L"CameraNum : & %d\n FrameNum : %d\n ObjectNum : %d\n TechnologyType : %d", packet->CameraNumber, packet->FrameNumber, packet->NumberOfObject, packet->TechnologyType);
	AfxMessageBox(msg);
	msg.Format(L"x : & %d\n y : %d\n w : %d\n h : %d", rcvObject->x, rcvObject->y, rcvObject->Width, rcvObject->Height);
	AfxMessageBox(msg);*/
